...
interface IAgentReward {
    ...
    event NewMainReward(uint32 indexed pos, uint256 amount, uint256 agentCount, uint256 totalStaked);
    ...
    event RefContractsUpdated(address rewardToken, address agentNft, address contributionNft, address serviceNft);

    event StakeThresholdUpdated(uint256 threshold);

    event ParentSharesUpdated(uint256 shares);

    event StakerRewardClaimed(uint256 virtualId, uint256 amount, address staker);

    event ValidatorRewardClaimed(uint256 virtualId, uint256 amount, address validator);

    event ServiceRewardsClaimed(uint256 nftId, address account, uint256 total, uint256 childrenAmount);
