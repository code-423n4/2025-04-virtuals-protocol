...
contract AgentTax is Initializable, AccessControlUpgradeable {
    ...
    event SwapExecuted(uint256 indexed agentId, uint256 taxTokenAmount, uint256 assetTokenAmount);
    ...
    event CreatorUpdated(uint256 agentId, address oldCreator, address newCreator);
    ...
    function initialize(
        ...
        emit SwapParamsUpdated2(address(0), router_, address(0), assetToken_, 0, feeRate, 0, creatorFeeRate);
    ...
    function _getTaxRecipient(uint256 agentId) internal returns (TaxRecipient memory) {
