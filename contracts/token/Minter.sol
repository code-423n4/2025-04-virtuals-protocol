...
contract Minter is IMinter, Initializable, OwnableUpgradeable {
    ...
    function setIPShareOverride(uint256 virtualId, uint256 _ipShare) public onlyAgentDAO(virtualId) {
    ...
    function setImpactMulOverride(uint256 virtualId, uint256 mul) public onlyOwner {
    ...
    function _getImpactMultiplier(uint256 virtualId) internal view returns (uint256) {
    ...
    function mint(uint256 nftId) public noReentrant {
        ...
        uint256 virtualId = IContributionNft(contributionNft).tokenVirtualId(nftId);
