...
library RewardSettingsCheckpoints {
    ...
    function push(Trace storage self, uint32 key, RewardSettings memory value) internal {
        return _insert(self._checkpoints, key, value);
    }

    function _insert(Checkpoint[] storage self, uint32 key, RewardSettings memory value) private {
    ...
    function latest(Trace storage self) internal view returns (RewardSettings memory) {
        uint256 pos = self._checkpoints.length;
        return pos == 0 ? RewardSettings(0, 0, 0, 0, 0) : self._checkpoints[pos - 1]._value;
    }

    function upperLookupRecent(Trace storage self, uint32 key) internal view returns (RewardSettings memory) {
