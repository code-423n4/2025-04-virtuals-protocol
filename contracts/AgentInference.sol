...
contract AgentInference is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Prompt(address indexed sender, bytes32 promptHash, uint256 agentId, uint256 cost, uint8[] coreIds);
    ...
    function prompt(
        ...
        require(agentIds.length == amounts.length && agentIds.length == coreIds.length, "Invalid input");
    ...
    function promptMulti(
        ...
        require(len == amounts.length && len == coreIds.length && len == promptHashes.length, "Invalid input");
