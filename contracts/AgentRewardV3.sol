...
contract AgentRewardV3 is IAgentRewardV3, Initializable, AccessControl, TokenSaver {
    ...
    function getAgentReward(uint256 virtualId, uint256 pos) public view returns (AgentReward memory) {
    ...
    function distributeRewards(
        ...
        RewardSettingsCheckpointsV2.RewardSettings memory settings = getRewardSettings();

        uint256 protocolAmount = shouldShareWithProtocol ? _distributeProtocolRewards(amount) : 0;
        ...
        for (uint i = 0; i < virtualCount; i++) {
            ...
            _distributeAgentReward(virtualId, rewardIndex, (lpValues[i] * balance) / totalLPValues, settings);
        }
    }
    ...
    function _distributeAgentReward(
        ...
        uint256 totalProposals = IAgentDAO(nft.virtualInfo(virtualId).dao).proposalCount();

        _agentRewards[virtualId].push(
            AgentReward(agentRewardId, rewardIndex, stakerAmount, amount - stakerAmount, totalProposals, totalStaked)
        );
    ...
    function _distributeProtocolRewards(uint256 amount) private returns (uint256) {
        RewardSettingsCheckpointsV2.RewardSettings memory rewardSettings = _rewardSettings.latest();
    ...
    function getClaimableValidatorRewards(
        ...
        numRewards = Math.min(LOOP_LIMIT + claim.rewardCount, getAgentRewardCount(virtualId));
        IVotes veToken = IVotes(IAgentNft(agentNft).virtualLP(virtualId).veToken);
        IAgentDAO dao = IAgentDAO(IAgentNft(agentNft).virtualInfo(virtualId).dao);
        for (uint i = claim.rewardCount; i < numRewards; i++) {
            AgentReward memory agentReward = getAgentReward(virtualId, i);
            Reward memory reward = getReward(agentReward.rewardIndex);
            ...
            uint256 validatorReward = (agentReward.validatorAmount * votes) / agentReward.totalStaked;
            validatorReward = (validatorReward * uptime) / agentReward.totalProposals;
    ...
    function getTotalClaimableStakerRewards(
        ...
        for (uint i = 0; i < virtualIds.length; i++) {
            uint256 virtualId = virtualIds[i];
            (uint256 claimable, ) = getClaimableStakerRewards(account, virtualId);
    ...
    function getTotalClaimableValidatorRewards(
        ...
        for (uint i = 0; i < virtualIds.length; i++) {
            uint256 virtualId = virtualIds[i];
            (uint256 claimable, ) = getClaimableValidatorRewards(account, virtualId);
            totalClaimable += claimable;
        }
    }

    function getAgentRewardCount(uint256 virtualId) public view returns (uint256) {
        return _agentRewards[virtualId].length;
    }
    ...
    function claimStakerRewards(uint256 virtualId) public noReentrant {
        ...
        uint256 numRewards;
        (totalClaimable, numRewards) = getClaimableStakerRewards(account, virtualId);
    ...
    function claimValidatorRewards(uint256 virtualId) public noReentrant {
        ...
        uint256 totalClaimable;
        uint256 numRewards;
        (totalClaimable, numRewards) = getClaimableValidatorRewards(account, virtualId);
    ...
    function claimAllStakerRewards(uint256[] memory virtualIds) public noReentrant {
        address account = _msgSender();
        uint256 totalClaimable;
        for (uint i = 0; i < virtualIds.length; i++) {
            uint256 virtualId = virtualIds[i];
            uint256 claimable;
            uint256 numRewards;
            (claimable, numRewards) = getClaimableStakerRewards(account, virtualId);
            totalClaimable += claimable;
    ...
    function claimAllValidatorRewards(uint256[] memory virtualIds) public noReentrant {
        address account = _msgSender();
        uint256 totalClaimable;
        for (uint i = 0; i < virtualIds.length; i++) {
            uint256 virtualId = virtualIds[i];
            uint256 claimable;
            uint256 numRewards;
            (claimable, numRewards) = getClaimableValidatorRewards(account, virtualId);
            totalClaimable += claimable;
    ...
    function setRewardSettings(uint16 protocolShares_, uint16 stakerShares_) public onlyGov {
        _rewardSettings.push(
            ...
            RewardSettingsCheckpointsV2.RewardSettings(protocolShares_, stakerShares_)
        );

        emit RewardSettingsUpdated(protocolShares_, stakerShares_);
    }

    function updateRefContracts(address rewardToken_, address agentNft_) external onlyGov {
        rewardToken = rewardToken_;
        agentNft = agentNft_;
